...
export const ThreeDColumnChart = ({ members = [], currency = 'INR', totalSpending = 0 }) => {
  ...
  return (
    <motion.div
      ...
      className="finlance-card p-4 sm:p-8 rounded-3xl border border-slate-200 bg-white shadow-md space-y-4 sm:space-y-6 overflow-hidden"
    >
      {/* Header */}
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4 border-b border-slate-100 pb-4">
        <div className="flex items-center space-x-3">
          <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-2xl bg-gradient-to-tr from-purple-600 via-indigo-600 to-blue-500 text-white flex items-center justify-center font-bold shadow-md shadow-indigo-500/20 shrink-0">
            <Sparkles className="w-4 h-4 sm:w-5 sm:h-5" />
          </div>
          <div>
            <h3 className="font-space text-sm sm:text-lg font-extrabold text-slate-900 tracking-tight">
        ...
        <div className="flex items-center space-x-2 text-[11px] sm:text-xs font-space font-extrabold px-3 py-1 sm:px-3.5 sm:py-1.5 rounded-full bg-slate-100 text-slate-700 border border-slate-200 shrink-0">
      ...
      <div className="relative pt-6 sm:pt-8 pb-3 sm:pb-4 px-2 sm:px-6 rounded-3xl bg-gradient-to-b from-indigo-50/70 via-slate-50 to-purple-50/50 text-slate-900 border border-slate-200 shadow-inner overflow-hidden">
        ...
        {/* 3D Columns Container (Scrollable on small screens if squeezed, centered & spread on larger screens) */}
        <div className="relative z-10 flex items-end justify-start sm:justify-around gap-2.5 sm:gap-6 min-h-[250px] sm:min-h-[320px] pt-10 sm:pt-12 pb-3 overflow-x-auto scrollbar-none px-1">
          {sortedMembers.map((m, idx) => {
            ...
            return (
              <div
                key={m._id || idx}
                className="flex-1 flex flex-col items-center group relative min-w-[70px] sm:min-w-[85px] max-w-[95px] sm:max-w-[110px] shrink-0 sm:shrink"
              >
                ...
                <motion.div
                  ...
                  className="mb-2 sm:mb-3 text-center transition-transform group-hover:-translate-y-1 z-20 w-full flex flex-col items-center"
                >
                  <span className="font-space text-[9px] sm:text-xs font-extrabold px-1.5 sm:px-2.5 py-0.5 sm:py-1 rounded-lg sm:rounded-xl bg-white/95 backdrop-blur-md text-slate-900 border border-slate-200/90 shadow-md block whitespace-nowrap max-w-full truncate">
                    {formatCurrency(paid, currency)}
                  </span>
                  {isHighest && (
                    <span className="text-[8px] sm:text-[9px] font-space font-extrabold text-amber-700 flex items-center justify-center space-x-0.5 sm:space-x-1 mt-0.5">
                ...
                <div className="w-full relative flex flex-col justify-end" style={{ height: `${Math.max(28, (heightPct / 100) * 170)}px` }}>
                  {/* Top 3D Cap */}
                  <motion.div
                    initial={{ scale: 0 }}
                    whileInView={{ scale: 1 }}
                    ...
                    className={`w-full h-3 sm:h-3.5 rounded-t-lg ${palette.top} shadow-sm border-t border-white/60`}
                  />
                  ...
                  >
                    ...
                    <div className="absolute inset-y-0 left-0 w-1.5 sm:w-2 bg-white/35 backdrop-blur-xs" />
                  </motion.div>
                </div>
                ...
                <motion.div
                  ...
                  className="mt-3 sm:mt-4 text-center space-y-0.5 sm:space-y-1 z-10 w-full"
                >
                  <div className="relative inline-block">
                    <img
                      src={m.avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${encodeURIComponent(m.name)}`}
                      alt={m.name}
                      className="w-7 h-7 sm:w-9 sm:h-9 rounded-full border-2 border-white bg-white object-cover mx-auto shadow-md"
                    />
                  </div>

                  <p className="font-space text-[11px] sm:text-xs font-bold text-slate-800 truncate w-full px-0.5" title={m.name}>
                    {m.name}
                  </p>
                  <span className="text-[9px] sm:text-[10px] font-space text-slate-500 block font-bold">
